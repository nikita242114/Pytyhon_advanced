...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Получаем операцию над двумя матрицами"
    )
    parser.add_argument(
        "-m_1",
        type=list,
    ...
    if args.operation == "+":
        print(
            f"СЛОЖЕНИЕ: {m_1} {args.operation} {m_2} : ",
            (f"{Matrix(m_1) + Matrix(m_2)} "),
        )
    elif args.operation == "*":
        print(
            f"УМНОЖЕНИЕ: {m_1} {args.operation} {m_2} : ",
            (f"{Matrix(m_1) * Matrix(m_2)} "),
        )
    elif args.operation == "=":
        print(
            f"РАВЕНСТВО: {m_1} {args.operation} {m_2} : ",
            (f"{Matrix(m_1) == Matrix(m_2)} "),
        )
    else:
